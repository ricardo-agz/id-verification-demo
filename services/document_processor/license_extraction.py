...
class LicenseData(BaseModel):
    first_name: FieldExtraction
    last_name: FieldExtraction
    birth_date: FieldExtraction
    expiration_date: FieldExtraction
    license_number: FieldExtraction
    address: FieldExtraction
    state: FieldExtraction
    sex: FieldExtraction
    height: FieldExtraction
    eye_color: FieldExtraction
    rstr: FieldExtraction


class LicenseDataResponse(BaseModel):
    image_analysis: str
    license_data: LicenseData


async def extract_license_data(image_path: str) -> LicenseDataResponse:
    ...
    response = await query_llm_with_fallbacks(
        ...
        response_schema=LicenseDataResponse,
        temperature=0.0,
        max_tokens=1000,
        messages=[
            {
                "role": "system",
                "content": """You are a precise document scanner specialized in extracting information from driver's 
                            licenses. 
                            ...
                            For addresses, include the full street address, city, and ZIP code if visible.
                            Be precise with license numbers - do not guess if digits are unclear.
            ...
            },
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": "Extract the license information and mark your confidence for each field.",
                    },
                ],
            },
        ],
...
async def main():
    data = await extract_license_data("../../test_images/test-license.png")
    print(type(data))
    print(data)
